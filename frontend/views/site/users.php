...
$this->title = 'Иполнители | TaskForce';
?>
        ...
        <div class="content-view__feedback-card user__search-wrapper">
            <div class="feedback-card__top">
                <div class="user__search-icon">
                    ...
                    <span><?= Yii::$app->i18n->format('{n, plural, one{# задание} few{# задания} many{# заданий} other{# заданий}}', ['n' => count($user->tasksAssigned)], 'ru_RU'); ?></span>
                    <span><?= Yii::$app->i18n->format('{n, plural, one{# отзыв} few{# отзыва} many{# отзывов} other{# отзывов}}', ['n' => count($user->opinionsGot)], 'ru_RU'); ?></span>
                </div>
                <div class="feedback-card__top--name user__search-card">
                    <p class="link-name"><a href="#" class="link-regular"><?= $user->name ?></a></p>
                    <?php
                    $opinions = $user->opinionsGot;
                    $rates = [];
                    $raiting = 0;
                    ...
                    if (!empty($rates)) {
                        $raiting = round(array_sum($rates) / count($rates), 2);
                    }
                    ?>
                    <?php for ($i = 1; $i <= 5; $i++) {
                        echo ($i <= $raiting ? '<span>' : '<span class="star-disabled">') . '</span>';
                    } ?>
                    <b><?= $raiting ? $raiting : '' ?></b>
                    <p class="user__search-content"><?= $user->profiles->about ?></p>
                </div>
            ...
            <div class="link-specialization user__search-link--bottom">
                <?php $chosenCategories = explode(', ', $user->profiles->categories);
